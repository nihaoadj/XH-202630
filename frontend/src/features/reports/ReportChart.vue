...
const radarOption = computed(() => ({
  tooltip: {
    trigger: 'item',
    formatter: () => dimensions.value.map((name, index) => `${name}：${radarStatuses.value[index] === 'unassessed' ? '待测' : `${values.value[index]}%`}`).join('<br/>'),
  },
  radar: { center: ['50%', '55%'], radius: '66%', splitNumber: 4, axisName: { color: '#657b94', fontSize: 11 }, splitLine: { lineStyle: { color: '#d9e5ef' } }, splitArea: { areaStyle: { color: ['rgba(243, 249, 255, .7)', 'rgba(255,255,255,.5)'] } }, axisLine: { lineStyle: { color: '#d9e5ef' } }, indicator: dimensions.value.map((name, index) => ({ name: radarStatuses.value[index] === 'unassessed' ? `${name}\n待测` : name, max: 100 })) },
