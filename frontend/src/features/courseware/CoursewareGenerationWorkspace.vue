  ...
  <div class="courseware-generation-page">
    <section class="workspace-header">
      <div>
        ...
        <h3>互动课件生成</h3>
        <p>选择若干已发布资源；系统会为每一份资源独立生成一份可互动的 HTML 课件。</p>
      </div>
    ...
    <section v-if="currentJob" class="courseware-job-summary">
    ...
    <section v-if="currentJob" class="generation-grid">
      <article class="process-panel">
        ...
        <el-steps direction="vertical" :active="activeStage" finish-status="success" process-status="process">
          <el-step v-for="stage in stages" :key="stage.id" :title="stage.label" :description="stageDescription(stage)" />
        </el-steps>
      ...
      <article class="details-panel">
        ...
        <section v-if="Object.keys(currentJob.request_options || {}).length" class="frozen-options"><strong>已冻结偏好</strong><el-tag v-for="(value, key) in currentJob.request_options" :key="key" effect="plain">{{ optionLabel(key) }}：{{ value }}</el-tag></section>
    ...
    <section v-else class="empty-workspace"><span class="eyebrow">Ready to create</span><h3>还没有可展示的互动课件任务</h3><p>选择已发布资源，为讲义、实操指南、测试题或复习清单分别生成互动版。</p></section>

    <el-dialog v-model="sourceDialogVisible" title="创建互动课件" width="min(680px, 92vw)" :close-on-click-modal="false">
      <p class="source-hint">每个选中的资源都会独立冻结、独立规划、独立生成。不会合并资源内容；实操指南生成逐步操作版，测试题生成答题与解析版。</p>
      <div class="preference-grid"><label><span>学习目标</span><el-input v-model="preferences.learning_goal" maxlength="240" placeholder="例如：完成本资源的核心练习" /></label><label><span>预计时长（分钟）</span><el-input-number v-model="preferences.expected_duration_minutes" :min="5" :max="240" :step="5" /></label><label><span>互动强度</span><el-select v-model="preferences.interaction_intensity"><el-option label="低" value="low" /><el-option label="中" value="medium" /><el-option label="高" value="high" /></el-select></label><label><span>视觉主题</span><el-select v-model="preferences.visual_style_id"><el-option label="编辑风" value="editorial" /><el-option label="午夜" value="midnight" /><el-option label="纸张" value="paper" /></el-select></label></div>
...
defineEmits(['open-text-workspace'])
...
const selectedLearnerId = ref(route.query.learnerId || localStorage.getItem('last_learner_id') || '')
...
const preferences = ref({ learning_goal: '', expected_duration_minutes: 30, interaction_intensity: 'medium', visual_style_id: 'editorial' })
...
function optionLabel(key) { return ({ learning_goal: '学习目标', expected_duration_minutes: '预计时长', interaction_intensity: '互动强度', visual_style_id: '视觉主题' }[key] || key) }
...
async function loadProfiles() { const [profileRes, domainRes] = await Promise.all([profileApi.list({ page: 1, page_size: 50 }), knowledgeApi.listDomains()]); profiles.value = profileRes.data.items || profileRes.data.profiles || []; tracks.value = (domainRes.data.domains || []).flatMap((domain) => domain.tracks || []); if (!profiles.value.some((item) => item.learner_id === selectedLearnerId.value)) selectedLearnerId.value = profiles.value[0]?.learner_id || '' }
async function loadSourceResources() { if (!selectedLearnerId.value) { sourceResources.value = []; return } const response = await resourceLibraryApi.listByLearner(selectedLearnerId.value); sourceResources.value = (response.data || []).filter((item) => item.resource_kind !== 'interactive_courseware').map((item) => ({ ...item, resource_id: item.resource_id || item.id })); selectDefaultSources() }
...
function openPublishedResource() { if (!currentJob.value?.resource_id) return; router.push({ path: '/resources', query: { learnerId: selectedLearnerId.value, runId: currentJob.value.source_batch_id || '', resourceId: currentJob.value.resource_id } }) }
