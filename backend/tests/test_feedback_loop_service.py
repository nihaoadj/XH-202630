...
def test_low_attempt_atomically_updates_profile_path_and_queues_followup():
    ...
    relation = service.feedback_loop_repo.get_followup_relation(result.followup_run_id)
...
def test_failed_followup_is_reconciled_with_stable_run_id_on_idempotent_retry():
    service, learners, jobs, profile, resource = _setup()
    service.generation_job_service = None
    request = _request(0.4)
    failed = service.process_learning_attempt(profile, resource, request)
    assert failed.followup_generation_status == "failed"

    service.generation_job_service = GenerationJobService(jobs, _NoopGenerationService())
    retried = service.process_learning_attempt(learners.get("learner"), resource, request)
    repeated = service.process_learning_attempt(learners.get("learner"), resource, request)

    assert retried.idempotent_replay is True
    assert retried.followup_generation_status == "queued"
    assert retried.followup_run_id == repeated.followup_run_id
    assert len(jobs.list_by_learner("learner")) == 1
