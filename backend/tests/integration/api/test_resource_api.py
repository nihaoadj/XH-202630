...
def test_resource_detail_and_html_preview_enforce_publication_and_lineage():
    learner_repo = MemoryLearnerRepository()
    learner_repo.save(LearnerProfile(
        learner_id="html_learner",
        learner_type="测试",
        education="本科",
        major="计算机",
        learning_goal="HTML 预览测试",
    ))
    resource_repo = MemoryResourceRepository()
    common = {
        "learner_id": "html_learner",
    ...
    text_resource = LearningResource(
        **common,
    ...
    html_resource = LearningResource(
        **common,
        resource_id="practice-html",
        representation="html",
        content_text='<section onclick="alert(1)"><script>bad()</script><p>安全内容</p></section>',
        storage_type="file",
        mime_type="text/html",
        derived_from_resource_id="practice-text",
        source_resource_version=1,
        review_status="approved",
        publication_status="published",
    )
    stale_html = html_resource.model_copy(update={
        "resource_id": "practice-html-stale",
        "version": 2,
        "source_resource_version": 2,
    })
    draft_html = html_resource.model_copy(update={
        "resource_id": "practice-html-draft",
        "version": 3,
        "publication_status": "unpublished",
        "review_status": "pending_review",
    })
    for item in (text_resource, html_resource, stale_html, draft_html):
        resource_repo.save(item, "html_learner", "检索")
    ...
    detail = client.get("/api/resources/items/practice-text")
    assert detail.status_code == 200
    assert detail.json()["resource"]["is_published"] is True
    assert client.get("/api/resources/items/practice-html-draft").status_code == 404
    assert client.get("/api/resources/items/practice-html-draft/preview").status_code == 404

    preview = client.get("/api/resources/items/practice-html/preview")
    assert preview.status_code == 200
    fragment = preview.json()["preview"]["html_fragment"]
    assert "安全内容" in fragment
    assert "<script" not in fragment
    assert "onclick" not in fragment
    assert client.get("/api/resources/items/practice-html-stale/preview").status_code == 409

    summary = client.get(
        "/api/resources/html_learner?page=1&page_size=2&summary_only=true&run_id=run-html-preview"
    )
    assert summary.status_code == 200
    assert summary.json()["total"] == 3
    assert summary.json()["has_next"] is True
    assert all(item["content_text"] is None for item in summary.json()["resources"])
