...
def test_strict_mode_keeps_reviewer_failure_fail_closed_without_failing_run(monkeypatch):
    monkeypatch.setattr(errors_module, "get_settings", lambda: _settings())
    gateway = LLMGateway(
    ...
    assert result["review_result"]["passed"] is False
    assert result["trace"][0]["status"] == "degraded"
    assert result["trace"][0]["error_code"] == ErrorCode.LLM_TIMEOUT.value
...
def test_reviewer_does_not_re_review_or_publish_generation_failure():
    state = _review_state()
    evidence = state["retrieved_evidence"][0]
    failed = state["generated_resources"][0].model_copy(update={
        "review_status": "human_review",
        "publication_status": "unpublished",
    })
    healthy = LearningResource(
        resource_id="healthy-resource",
        resource_type="分阶测试题",
        difficulty="初级",
        content_text="可正常审核的测试题",
        knowledge_points=["检索"],
        source_refs=source_refs_from_evidence([evidence]),
    )
    state.update({
        "generated_resources": [failed, healthy],
        "resource_executions": [
            {
            ...
            {
                "resource_id": healthy.resource_id,
                "resource_type": healthy.resource_type,
                "representation": "text",
                "resource_execution_state": "generated",
    ...
    result = review_node(state, llm_gateway=gateway)

    assert len(gateway.calls) == 1
    prompt = gateway.calls[0]["messages"][-1].content
    assert healthy.resource_id in prompt
    assert failed.resource_id not in prompt
    assert result["resource_review_results"][failed.resource_id]["decision"] == "human_review"
    assert result["resource_review_results"][healthy.resource_id]["decision"] == "approve"
    resources = {item.resource_id: item for item in result["generated_resources"]}
    assert resources[failed.resource_id].review_status == "human_review"
    assert resources[failed.resource_id].publication_status == "unpublished"
    assert resources[healthy.resource_id].review_status == "approved"
    assert resources[healthy.resource_id].publication_status == "published"


def test_finalizer_cannot_override_locked_human_review_execution():
    state = _review_state()
    resource = state["generated_resources"][0].model_copy(update={
    ...
    state.update({
        ...
        "review_result": {
            ...
            "status": "approve",
            "passed": True,
            "review_ids": {resource.resource_id: "review-locked"},
            "issues": [],
        },
        "resource_review_results": {
            resource.resource_id: {"decision": "approve"},
        },
        "resource_executions": [{
    ...
    finalized = result["generated_resources"][0]
    assert result["review_result"]["decision"] == "human_review"
    assert result["workflow_status"] == "human_review"
    assert finalized.review_status == "human_review"
    assert finalized.publication_status == "unpublished"
    assert result["resource_executions"][0]["resource_execution_state"] == "human_review"
