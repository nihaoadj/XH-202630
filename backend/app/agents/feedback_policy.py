...
def build_mastery_mutations(
    ...
    for item in attempt.knowledge_point_results:
        ...
        mastery = item.score if previous is None else 0.7 * previous.mastery + 0.3 * (item.score or 0.0)
        ...
        mutations.append(KnowledgeStateMutation(
            ...
            reason="EWMA(0.7 * previous + 0.3 * current_attempt); hints/duration excluded",
        ))
    return mutations
