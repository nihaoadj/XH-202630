...
目标配额是 3 道闭卷回忆、3 道概念辨析和 1 道正反例辨认。证据不足时可以减量，但每个未生成槽位
...
明确支持的一个关键边界。每题必须绑定至少一个当前节点 Evidence。不要输出 Markdown、代码围栏或额外文字。"""
    ...
    for index, block in enumerate(package["node_blocks"]):
        ...
        for label, key, kind in (("闭卷回忆", "recall_questions", "recall"), ("概念辨析", "distinction_questions", "distinction")):
            ...
            for item in block[key]:
                answers.append((item, kind)); lines += [f"#### {item['question_id']}", ""]
                lines += [f"判断下列陈述是否正确，并说明依据：{item['statement']}" if kind == "distinction" else item["prompt"], "", "[ ] 会  [ ] 模糊  [ ] 不会", ""]
        lines += ["### 正反例辨认", ""]
        item = block.get("example_recognition")
        if item:
            answers.append((item, "example")); lines += [f"#### {item['question_id']}", "", f"A. {item['candidate_a']}", "", f"B. {item['candidate_b']}", "", "判断哪个是正例、哪个是反例，并说明决定性差异。", "", "[ ] 会  [ ] 模糊  [ ] 不会", ""]
        else: lines += ["本节点证据未提供足以构成单一明确边界的正反例；请先完成上方回忆与辨析。", ""]
    lines += ["## 答案与证据解释", ""]
    for item, kind in answers:
        lines += [f"### {item['question_id']}", ""]
        ...
        lines += [f"解释：{item['explanation']}", "", f"达标标准：{item['pass_criteria']}", "", f"证据：{', '.join(item['evidence_ids'])}", ""]
...
class ReviewChecklistAgent(BaseResourceGenerationAgent[ReviewPracticeNodeBlockV2]):
    resource_type = "复习清单"; agent_name = "ReviewChecklistAgent"; prompt_version = "review-practice-v2-node-evidence"; artifact_format = "json"; validation_retry_attempts = 2
    @staticmethod
    ...
    def _validate_block(self, block: ReviewPracticeNodeBlockV2, spec: ResourceSpec, node_id: str) -> None:
        allowed = set(self._node_evidence_ids(spec, node_id)); questions = [*block.recall_questions, *block.distinction_questions, *([block.example_recognition] if block.example_recognition else [])]
        if block.skill_node_id != node_id or not allowed or not set(block.evidence_ids) <= allowed or any(not set(q.evidence_ids) <= allowed for q in questions): raise ApplicationError(ErrorCode.EVIDENCE_SCOPE_VIOLATION, status_code=422)
        ...
        for index, node_id in enumerate(spec.knowledge_points):
            ...
            for _ in range(self.validation_retry_attempts):
                try:
                    ...
                    self._validate_block(result.output, spec, node_id); block = result.output.model_dump(mode="json"); base=index*7
                    for item in block["recall_questions"]: item["question_id"] = f"q-{base+int(item['local_id'][-1]):03d}"
                    for item in block["distinction_questions"]: item["question_id"] = f"q-{base+3+int(item['local_id'][-1]):03d}"
                    if block.get("example_recognition"): block["example_recognition"]["question_id"] = f"q-{base+7:03d}"
