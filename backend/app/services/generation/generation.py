...
from app.core.storage.file_storage import save_text_resource
...
from app.models.learning_documents.schemas import GenerateRequest, GenerateResponse, LearnerProfile, LearningResource
...
class GenerationService:
    ...
    def generate_with_run_id(
        ...
        try:
            ...
            review_ids = {
                str(resource_id): str(review_id)
                for resource_id, review_id in (review.get("review_ids") or {}).items()
                if review_id
            }
            for resource in raw_resources:
                if resource.review_id is None and resource.resource_id in review_ids:
                    resource.review_id = review_ids[resource.resource_id]
