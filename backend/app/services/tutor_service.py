...
class TutorService:
    ...
    def _resolve_question_context(
        ...
        if question_id is None:
            return None
        questions = self.knowledge_service.load_diagnostic_questions(
        ...
        for resource in resources:
            for exercise in resource.exercise_items:
                if exercise.question_id == question_id:
                    return self._exercise_context(resource, exercise)
        raise ApplicationError(ErrorCode.TUTOR_CONTEXT_INVALID, status_code=422)
    ...
    def _exercise_context(
        ...
        return TutorQuestionContext(
            ...
            options=[],
            skill_node_id=resource.learning_path_node,
