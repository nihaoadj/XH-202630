...
class KnowledgeState(BaseModel):
    ...
    self_report_prior: Optional[float] = Field(default=None, ge=0.0, le=1.0)
    confidence: Optional[Literal["none", "low", "medium", "high"]] = None
...
class GenerateRequest(BaseModel):
    ...
    def normalize_resource_types(cls, values: List[str]) -> List[str]:
        from app.models.learning_documents.types import canonical_resource_type
...
class GenerationJobCreateResponse(StatusResponse):
    ...
    focus_snapshot: Optional[LearnerFocusSnapshotV1] = None
