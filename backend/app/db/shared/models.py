...
class KnowledgeStateORM(Base):
    ...
    state_schema_version = Column(String(16), nullable=False, default="2.0", server_default="2.0")
    self_report_prior = Column(Float, nullable=True)
    confidence = Column(String(16), nullable=False, default="none", server_default="none")
    objective_evidence_count = Column(Integer, nullable=False, default=0, server_default="0")
    distinct_objective_source_count = Column(Integer, nullable=False, default=0, server_default="0")
    last_evidence_type = Column(String(32), nullable=True)
    last_evidence_id = Column(String(128), nullable=True)
...
class AbilityStateEventORM(Base):
    __tablename__ = "ability_state_events"
    __table_args__ = (
        UniqueConstraint(
            "learner_id", "source_type", "source_id", "skill_node_id",
            name="uq_ability_event_source_node",
        ),
        Index("ix_ability_events_learner_time", "learner_id", "occurred_at", "event_id"),
    )

    event_id = Column(String(128), primary_key=True)
    schema_version = Column(String(16), nullable=False, default="1.0", server_default="1.0")
    learner_id = Column(String(64), ForeignKey("learner_profiles.learner_id"), nullable=False, index=True)
    knowledge_base_id = Column(String(128), ForeignKey("knowledge_bases.knowledge_base_id"), nullable=False, index=True)
    skill_node_id = Column(String(128), ForeignKey("rag_skill_nodes.node_id"), nullable=False, index=True)
