...
from app.models.learners.mastery import AbilityMasteryStateV2
        ...
        try:
            with self.session_factory() as db:
                ...
                for item in state_mutations:
                    ...
                    event_id = _stable_id("abe", attempt.attempt_id, item.knowledge_point_id)
                    row.evidence = list(dict.fromkeys([*(row.evidence or []), event_id]))[-20:]
                    row.state_schema_version = "2.0"
                    row.self_report_prior = item.after.self_report_prior
                    row.confidence = item.after.confidence
                    row.objective_evidence_count = item.after.objective_evidence_count
                    row.distinct_objective_source_count = item.after.distinct_objective_source_count
                    row.attempt_count = item.after.attempt_count
                    row.last_attempt_id = attempt.attempt_id
                    row.last_evidence_type = "learning_attempt"
                    row.last_evidence_id = event_id
                    ...
                    before_ability = AbilityMasteryStateV2(
                        learner_id=attempt.learner_id,
                        knowledge_base_id=profile.knowledge_base_id,
                        skill_node_id=item.knowledge_point_id,
                        mastery_score=item.before.mastery,
                        self_report_prior=item.before.self_report_prior,
                        status=item.before.status,
                        confidence=item.before.confidence,
                        objective_evidence_count=item.before.objective_evidence_count,
                        distinct_objective_source_count=item.before.distinct_objective_source_count,
                        attempt_count=item.before.attempt_count,
                        last_evidence_type=item.before.last_evidence_type,
                        last_evidence_id=item.before.last_evidence_id,
                        row_version=item.before.row_version,
                    ) if item.before else None
                    after_ability = AbilityMasteryStateV2(
                        learner_id=attempt.learner_id,
                        knowledge_base_id=profile.knowledge_base_id,
                        skill_node_id=item.knowledge_point_id,
                        mastery_score=item.after.mastery,
                        self_report_prior=item.after.self_report_prior,
                        status=item.after.status,
                        confidence=item.after.confidence,
                        objective_evidence_count=item.after.objective_evidence_count,
                        ...
                        last_evidence_id=event_id,
                        row_version=item.after.row_version,
                        last_updated=attempt.submitted_at,
                    )
                ...
                all_rows = db.query(KnowledgeStateORM).filter_by(
                    learner_id=attempt.learner_id,
                    knowledge_base_id=profile.knowledge_base_id,
                ).order_by(KnowledgeStateORM.skill_node_id).all()
                names = {
                    node.node_id: node.name
                    for node in db.query(RagSkillNodeORM).filter_by(
                        knowledge_base_id=profile.knowledge_base_id
                    ).all()
                }
                ...
                def display(node_id):
                    ...
                    return f"{name}（{node_id}）" if name_counts.get(name, 0) > 1 else name
                profile.knowledge_states = {
                    state.skill_node_id: {
                        "score": state.mastery_score,
                        "status": state.status or "unassessed",
                        "evidence": state.evidence or [],
                        "last_updated": state.last_updated.isoformat() if state.last_updated else None,
                        "self_report_prior": state.self_report_prior,
                        "confidence": state.confidence or "none",
                        "objective_evidence_count": state.objective_evidence_count or 0,
                        "distinct_objective_source_count": state.distinct_objective_source_count or 0,
                        "attempt_count": state.attempt_count or 0,
                        "last_evidence_type": state.last_evidence_type,
                        "last_evidence_id": state.last_evidence_id,
                        "row_version": state.row_version or 1,
                    }
                    for state in all_rows
                }
                profile.theory_scores = {
                    state.skill_node_id: round(float(state.mastery_score) * 100, 1)
                    for state in all_rows
                    if (state.objective_evidence_count or 0) > 0 and state.mastery_score is not None
                }
                profile.profile_version = profile_version.profile_version
                profile.skill_level = profile_patch["skill_level"]
                profile.weak_points = [
                    display(state.skill_node_id) for state in all_rows
                    if (state.objective_evidence_count or 0) > 0 and state.status == "weak"
                ]
                profile.strong_points = [
                    display(state.skill_node_id) for state in all_rows
                    if (state.objective_evidence_count or 0) > 0 and state.status == "mastered"
                ]
        ...
        *,
        ...
        with self.session_factory() as db:
            ...
            for attempt, decision, relation in rows:
                if relation and relation.child_run_id in stale and relation.status == "queued":
