...
| F09 | 学习报告 | 已完成可视化增强版 | 提供结果摘要、全节点学习节点掌握图、资源难度匹配和学习路径图；所有图表均从规范掌握度、正式 Attempt、已发布资源和持久化路径只读派生 |
...
- 低分默认激活降阶补救节点，中分默认推荐纠错包但仍开放用户自选可学习节点，高分默认进入进阶学习（优先一旧一新，无旧节点时两个新节点）；纠错包三档始终可选。
